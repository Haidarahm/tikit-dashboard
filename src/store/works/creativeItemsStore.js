...
export const useCreativeItemsStore = create((set, get) => ({
  ...
  fetchList: async (slug = null) => {
    ...
    try {
      const resp = await getAllCreativesItems({
        slug: targetSlug,
        page,
        per_page: perPage,
        lang,
      ...
      set({ error });
      toast.error(
        error?.response?.data?.message || "Failed to fetch creative items"
      );
    } finally {
      set({ isLoading: false });
    }
  ...
  create: async (payload) => {
    set({ isLoading: true, error: null });
    try {
      const created = await createCreativeItem(payload);
      set({ current: created?.data || created });
      await get().fetchList();
      toast.success("Creative item created successfully");
      return created;
    } catch (error) {
      set({ error });
      toast.error(
        error?.response?.data?.message || "Failed to create creative item"
      );
      throw error;
    } finally {
      set({ isLoading: false });
  ...
  update: async (id, payload) => {
    set({ isLoading: true, error: null });
    try {
      const updated = await updateCreativeItem(id, payload);
      set({ current: updated?.data || updated });
      await get().fetchList();
      toast.success("Creative item updated successfully");
      return updated;
    } catch (error) {
      set({ error });
      toast.error(
        error?.response?.data?.message || "Failed to update creative item"
      );
      throw error;
    } finally {
      set({ isLoading: false });
