...
export function deriveGalleryImageItems(project) {
...
export function deriveVideoItems(project) {
  if (Array.isArray(project?.media) && project.media.length > 0) {
    return project.media
...
export function buildShowcasePayload(values, logoList, imagesList, videosList) {
 ...
 */
export async function translateShowcaseFields(translateText, fields) {
  const {
    title_en,
    subtitle_en,
    objective_en,
...
export function parseEngagementRate(project) {
