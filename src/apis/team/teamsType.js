...
export async function getAllTeamTypes(params = {}) {
  ...
  const { data } = await apiClient.get("/api/team-types/get", {
    params: query,
  });
  return data;
}
...
export async function createTeamType(input) {
  ...
  const { data } = await apiClient.post("/api/team-types/add", payload);
  return data;
}
...
export async function deleteTeamType(id) {
  const { data } = await apiClient.delete(`/api/team-types/${id}/delete`);
  return data;
}
